...
function ManageBookingModal({ isOpen, onClose }) {
  ...
  const handleClose = () => {
    setBookingId(""); setBooking(null); setError(""); onClose();
  };
  ...
  const statusConfig = {
    confirmed:    { color: "bg-emerald-50 border-emerald-200 text-emerald-700", icon: "✅", label: "Confirmed" },
    pending:      { color: "bg-amber-50 border-amber-200 text-amber-700",       icon: "⏳", label: "Pending" },
    cancelled:    { color: "bg-red-50 border-red-200 text-red-600",             icon: "❌", label: "Cancelled" },
    "checked-out":{ color: "bg-blue-50 border-blue-200 text-blue-700",          icon: "🏁", label: "Checked Out" },
  };
  const st = booking ? (statusConfig[booking.status] || statusConfig.pending) : null;
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden pointer-events-auto max-h-[90vh] overflow-y-auto">
              ...
              <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-slate-100 sticky top-0 bg-white z-10">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-lg font-extrabold text-slate-900">Manage Booking</h2>
                </div>
              ...
              <div className="px-5 pt-5 pb-4">
                ...
                <div className="flex gap-2">
                  <div className="flex-1 flex items-center border border-slate-200 rounded-2xl overflow-hidden bg-slate-50 focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
                    ...
                    <input
                      ...
                      className="flex-1 bg-transparent px-3 py-3 text-sm text-slate-700 placeholder-slate-400 outline-none font-medium"
                    />
                  </div>
                  <button
                    onClick={handleFind}
                    disabled={loading}
                    className="bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white font-bold px-4 py-2.5 rounded-2xl text-sm transition-all shadow-sm flex items-center gap-2"
                  >
                    {loading ? (
                ...
                <p className="text-[11px] text-slate-400 mt-2 font-medium">
                  Your Booking ID was shown after submitting the booking form.
                </p>
              ...
              {booking && (
                <div className="px-5 pb-6 space-y-4">
                  {/* Status banner */}
                  <div className={`flex items-center gap-3 px-4 py-3 rounded-2xl border ${st.color}`}>
                    <span className="text-xl">{st.icon}</span>
                    <div>
                      <p className="font-extrabold text-sm">{st.label}</p>
                      <p className="text-xs font-medium opacity-80">
                        {booking.status === "confirmed"
                          ? "Your booking has been confirmed! Show your Booking ID at reception."
                          : booking.status === "pending"
                          ? "Your booking is pending confirmation."
                          : booking.status === "cancelled"
                          ? "This booking has been cancelled."
                          : "Checkout complete. Hope you enjoyed your stay!"}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between py-2 border-b border-slate-100">
                    ...
                    <span className="font-extrabold text-slate-800 text-sm tracking-widest">#{bookingId}</span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                    {[
                      ...
                      ["Phone",       booking.phone ? `+91 ${booking.phone}` : "—"],
                      ["Room Type",   booking.roomType || booking.roomId || "—"],
                      ["Guests",      booking.guests || "—"],
                      ["Check-In",    fmtDate(booking.checkIn)],
                      ["Check-Out",   fmtDate(booking.checkOut)],
                      ["Booked On",   fmtDate(booking.createdAt)],
                      ["Total",       booking.totalPrice ? `₹${Number(booking.totalPrice).toLocaleString("en-IN")}` : "—"],
                    ].map(([label, value]) => (
                      <div key={label}>
                  ...
                  <div className="flex gap-3 pt-2">
                    <a
                      href={`https://wa.me/919003549849?text=Hi!%20I%20want%20to%20cancel%20my%20booking.%20Booking%20ID:%20${bookingId}`}
                      target="_blank" rel="noopener noreferrer"
                      className="flex-1 text-center py-2.5 rounded-xl border border-red-300 text-red-500 hover:bg-red-50 font-bold text-xs transition-all"
                    >
                      Cancel Booking
                    </a>
                    <a
                      href={`https://wa.me/919003549849?text=Hi!%20I%20want%20to%20modify%20my%20booking.%20Booking%20ID:%20${bookingId}`}
                      target="_blank" rel="noopener noreferrer"
                      className="flex-1 text-center py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs transition-all shadow-sm"
                    >
                      Modify Booking
                    </a>
                  </div>
