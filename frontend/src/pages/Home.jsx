    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          // Video visible — play with sound
          video.muted = false;
          video.play().catch(() => { video.muted = true; video.play(); });
        } else {
          // Video off screen — pause and mute
          video.pause();
          video.muted = true;
        }
      },
      { threshold: 0.5 } // 50% visible-ஆகும்போது trigger
    );
  ...
  return (
    <div className="bg-white min-h-screen font-jakarta text-slate-800">
      ...
      <section ref={videoSectionRef} className="relative w-full overflow-hidden bg-black" style={{ height: "100svh", maxHeight: "700px" }}>
        ...
        {/* SM Golden Resorts text — fixed top center over video */}
